...
\subsection{Angabe von Gitterauflösungen}

Die geographischen Lage eines Punktes auf der Erdoberfläche wird meist
durch die geographische Breite (\textit{Latitude}) und Länge
(\textit{Longitude}) angegeben, und die Distanz zwischen zwei Punkten
in Kilometern bzw. Meilen. Insbesondere bei der Distanz denken die
meisten Menschen in einer Längeneinheit an die sie gewöhnt sind und
unter der sie sich etwas vorstellen können. 

Die hier betrachteten Vorhersagemodelle verwenden zur Festlegung von
Punkten auf der Erde ein Gitternetz, dessen Grundlage parallel zum
Äquator verlaufende Breiten- und von Pol zu Pol verlaufende
Längenkreise sind, die in der Einheit Grad angegeben werden. Bei der
Umrechnung der Gitterauflösung in Kilometer variiert deren Abstand
aufgrund der elipsoiden Gestalt der Erdkugel. Ein Grad Latitude
entspricht dabei durchschnittlich 111 Kilometern und die Umrechnung
ist weitestgehend konstant. Der Abstand zwischen den Längengraden
variiert allerdings erheblich. Am Äquator beträgt der Abstand
...
Einschätzung der Distanz zu verstehen.
...
den gesamten Globus umfassende Datenmenge auf die \textit{Spots}
reduziert, und alle relevanten Datensätze in einer \textit{CSV}
\nomenclature{CSV}{Comma-Separated Values} Datei gespeichert, die im
nächsten Schritt mit dem \textit{Bulk Loader} des Datenbank Management
Systems importiert werden kann. Zuvor muss allerdings noch auf eine
Eigenart des \textit{Wave Watch III} Modells eingegangen werden, damit
sich die Vorhersagedaten sinnvoll und zuverlässig mit \textit{Spots}
assoziieren lassen.
...
liefert deshalb nur für diejenigen Positionen Vorhersagedaten, die
sich auch über dem Meer befinden. Für alle anderen Positionen stehen
keine Daten zur Verfügung.

\begin{figure}[h]
  \begin{center}
    \includegraphics[width=\textwidth]{bilder/locate-position}
    \caption{Datenlücken im Wave Watch III Gitternetze}
