...
Wert auf die \textit{ACID} \nomenclature{ACID}{Atomicity, Consistency,
  Isolation, Durability} Eigenschaften eines Datenbank Management
Systems gelegt zu werden, da \textit{ActiveRecord} nach fast 4 Jahren
immer noch keine Methoden zur Definition von Fremdschlüsseln
bietet. Auch in der Dokumentation und in Diskussionen werden die
Methoden zur Gewährleistung referenzieller Integrität meist
ignoriert. Dieses Defizit wurde durch die Definition entsprechender
Methoden in der \textit{ActiveRecord} Bibliothek behoben, so dass
Fremdschlüssel direkt über die API definiert werden können. Diese
stellen die referenzielle Integrität der Daten sicher. Bei der
Entwicklung traten so einige Fehler sehr viel schneller zum Vorschein,
als wenn man auf die Werkzeuge zur Sicherung der referenzielle
Integrität verzichtet hätte.
...
die beiden Modelle erstellt wurden, und ist im Moment bei beiden
Modellen identisch. Die beiden Attribute
\textit{created\textunderscore at} und \textit{updated\textunderscore
  at} sind zwei von \textit{ActiveRecord} automatisch verwaltete
Attribute, welche die Zeitpunkte an dem der Datensatz erstellt und an
dem er zuletzt aktualisiert wurde enthalten. Alle anderen Attribute
repräsentieren die Vorhersageelemente der beiden Modelle und nehmen
die mit \textit{degrib} ausgelesenen Werte auf.
...
\subsubsection{Aktualisierung der Vorhersagedaten durch einzelne SQL
  Befehle}
Eine offensichtliche Methode die Vorhersagedaten mit \textit{SQL}
Befehlen zu aktualisieren, ist durch die Datensätzen der \textit{CSV}
Datei zu iterieren, und für jeden Datensatz eine \textit{UPDATE}
Operation, gefolgt von einer konditionalen \textit{INSERT} Operation
auszuführen. Da für einen bestimmten Vorhersagezeitpunkt eines Spots
bereits ein Datensatz aus einer früheren Modellberechnung existieren
kann, muss zunächst ein \textit{UPDATE} Befehl ausgeführt
werden. Durch die Angabe des Schlüsselkandidaten in der \textit{WHERE}
Klausel des Befehls wird die Menge der zu aktualisierenden Datensätze
auf genau einen beschränkt. Das Ergebnis des Befehls ist die Anzahl
der aktualisierten Datensätze und beträgt entweder 0 oder 1. Wurde ein
...
überhaupt keine Vorhersagedaten in der Datenbank sind. Da die
Vorhersagezeitpunkte zweier aufeinander folgenden Modellberechnungen
immer nur um 6 Stunden \footnote{in einem 3-stündigen
  Vorhersageintervall} voneinander abweichen, reicht für den Großteil
der Datensätze eine \textit{UPDATE} Operation aus. Eine zusätzliche
\textit{INSERT} Operation ist nur für diejenigen Datensätze nötig, die
in dem abweichenden Zeitraum liegen. Im Durchschnitt liegt die Anzahl
der Befehle somit leicht über $n$.

Die hier beschriebene Methode ist selbst mit möglichen Verbesserungen
wie z.B. der Verwendung von \textit{Prepared Statements}, dem
Ausschalten von \textit{Auto-Commit} und der Einbettung aller Befehle
in eine einzelne Transaktion nicht sehr effizient. Die Anzahl der
auszuführenden Befehle steigt hier mit der Zahl der Datensätze, und
der vom \textit{DBMS} betriebene Overhead wirkt sich negativ auf die
Performanz aus. In den folgenden zwei Abschnitten wird ein
performanteres Verfahren zur Aktualisierung der Vorhersagedaten
vorgestellt, das unter anderem auch im Bereich des \textit{Data
  Warehousing} eingesetzt wird.
...
arbeiten. Die \textit{SQL} Befehle zur Manipulation von Tupeln können
aber auch mengenorientiert eingesetzt werden, so dass pro Befehl mehr
als nur ein Tupel geändert bzw. hinzugefügt wird. Voraussetzung
hierfür ist aber, dass das \textit{DBMS} auf alle zu verarbeitenden
Daten zugreifen kann. Die war bei der vorigen Methode nicht der Fall,
da das \textit{DBMS} bei jeder Operation nur einen einzelnen Datensatz
aus der Eingabe zu Gesicht bekam.
