...
auszuführenden Befehle steigt hier mit der Anzahl der Datensätze, und
der vom \textit{DBMS} betriebene Overhead beim Ausführen der einzelnen
Befehle wirkt sich negativ auf die Performanz aus. Stattdessen wird
ein Verfahren angewendet, bei dem die Anzahl der Befehle konstant
bleibt und aus dem Bereich des \textit{Data Warehousing} bekannt ist.

\subsection{Bulk Loading}

Normalerweise werden Datensätze einer Datenbank mit den
standardisierten \textit{UPDATE} und \textit{INSERT} Befehlen der
\textit{Structured Query Language (SQL)} aktualisiert,
bzw. hinzugefügt.
