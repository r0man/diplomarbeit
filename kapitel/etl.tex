...
hinzugefügt. Es werden nur Datensätze aktualisiert bzw hinzugefügt,
...
Alternative hier ebenfalls nicht weiter verfolgt.

\subsubsection{Hinzufügen von Datensätzen}
Die hier implementierte Aktualisierung erfolgt in zwei Schritten. Im
ersten Schritt wird durch das Hinzufügen entsprechender Datensätze
sichergestellt, dass für jeden Spot und jeden Vorhersagezeitpunkt der
Quellrelation ein Datensatz in der Zielrelation existiert. Diese
hinzugefügten Datensätze dienen als Platzhalter und werden im zweiten
Schritt mit den Daten der Quellrelation aktualisiert. Mit einer
\textit{SQL} Anfrage wird diejenige Menge an Datensätze erzeugt, deren
Schlüsselkandidaten noch nicht in der Zielrelation existieren, sich
aber aus den Datensätzen der Quellrelation ableiten lassen. Diese
Datensätze dienen als Platzhalter und enthalten nur gültige Werte in
den Attributen \textit{spot\textunderscore id},
...
repräsentieren die Vorhersageelemente und werden vorübergehend mit dem
Wert \textit{NULL} belegt.

\subsubsection{Left Join vs. Subquery}
Die Menge der hinzuzufügenden Datensätze kann durch Anwendung
verschiedener \textit{SQL} Konstrukte erzeugt werden. Eine der
Aufgaben des Datenbank Management Systems ist es für eine Anfrage den
besten Ausführungsplan zu finden. Idealerweise sollten dabei zwei
äquivalente Befehle in den optimalen Ausführungsplan übersetzt
...
gut gelöst. Durch die Verwendung verschiedener \textit{SQL} Konstrukte
...
äquivalenten \textit{SQL} Befehlen durch die Analyse der
...
die Statistiken der Tabellen aktualisiert. Auf den aus den beiden
Attributen \textit{spot\textunderscore id} und
\textit{valid\textunderscore time} bestehenden Schlüsselkandidaten
wurden zusätzlich noch zusammengesetzte Indizes definiert.

\paragraph{Left Join}
In Auflistung \ref{forecasts:insert_1} ist die \textit{Left Join}
Variante zu sehen, mit der die Menge der hinzuzufügenden Datensätze
erzeugt wird. Der Befehl verknüpft die Datensätze der Relation
\textit{grib\textunderscore messages} über die Attribute
\textit{spot\textunderscore id} und \textit{valid\textunderscore time}
mit den Datensätzen der Relation \textit{forecasts}. Durch die
Bedingung in der \textit{WHERE} Klausel wird das Ergebnis auf die
Menge von Datensätzen reduziert, deren Schlüsselkandidat noch nicht in
der Relation \textit{forecasts} existiert.

\begin{lstlisting}[captionpos=b, caption=Einfügen von Platzhalter Datensätze mittels Left Join, label=forecasts:insert_1]
...
\ref{forecasts:insert_2} sehen, dass kein Index benutzt wird, sondern
ein \textit{Sequence Scan} auf den beiden Relationen
\textit{forecasts} und \textit{grib\textunderscore messages}
durchgeführt wird. Die Ausführungszeit des Befehls beträgt 214
Millisekunden.

\begin{lstlisting}[captionpos=b, caption=Ausführungsplan des Left Joins, label=forecasts:insert_2]
...
Zwar kann die Laufzeit von 214 Millisekunden noch als akzeptabel
eingeschätzt werden, falls aber historische Daten von vielen Spots
über einen längeren Zeitraum behalten werden, könnte man hier auf
Dauer ein Performanzproblem bekommen, da die \textit{forecasts}
Relation stetig anwachsen würde. Selbst das erzwungene Benutzen von
Indizes ergab bei dieser Anfrage ein schlechteres Laufzeitverhalten,
weshalb der vom Optimierer ausgewählte Plan als akzeptabel eingestuft
wird.
