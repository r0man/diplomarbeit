...
\item Bei der Initialisierung des Algorithmus wird die maximale
  ...
  $\alpha < 360^{\circ}$, mit der die zu untersuchenden Richtungen
  berechnet werden.

\item Zunächst werden die Wellenhöhen an der Ursprungsposition des
  Spots mit dem Programm \textit{degrib} ausgelesen. Falls alle Werte
  an dieser Position gültig sind (nicht 9999.0), ist der Algorithmus
  fertig und die ursprüngliche Position des Spots kann als
...
\item Nach der Erhöhung des Radius werden bestimmte geographische
  ...
  Positionen gültige Werte gefunden wurden, kann diese als
...
alternative Vorhersageposition wurde genau dann gefunden, wenn alle
Werte in der Ausgabe von \textit{degrib} gültig (nicht 9999.0)
sind. Die Anzahl der Aufrufe des Programms \textit{degrib} ist mit den
bei der Initialisierung festgelegten Parametern durch die Obergrenze
$1 + (360^{\circ} / \alpha) * (\Delta / \delta)$ beschränkt. Da die
Wahl der Parameter die Laufzeit und die Genauigkeit des Algorithmus
...
Der Algorithmus wurde auf 68 Spots angewendet, von denen zwei in Nord
...
anbietet. Für diese beiden Spots wurde das Programm \textit{degrib}
vom Algorithmus $1 + (360^{\circ} / 10^{\circ}) * (100km / 1km) =
3601$ mal ohne Erfolg aufgerufen. Für alle anderen Spots konnte mit
dem Algorithmus eine alternative Vorhersageposition ermitteln werden,
die zwischen 3 und 80 Kilometern von der Ursprungsposition entfernt
...
Programmaufrufe zu reduzieren wurde hier ein Verfahren angewendet, das
dem einer binären Suche ähnelt und als \textit{Divide and Conquer}
...
\item Bei der Initialisierung wird wieder die maximale Distanz
  ...
  Schrittweite $\alpha < 360^{\circ}$, mit der die verschiedenen
  Richtungen auf dem Umkreis des untersuchenden Radius berechnet
  werden. Außerdem wird ein Schwellwert $d$ benötigt, der als
  Abbruchkriterium verwendet wird und im 3. Schritt näher erklärt
  wird.

\item Da bei vielen Spots bereits an der Ursprungsposition
  Vorhersagedaten vorhanden sind, wird zunächst diese Position auf
  gültige Werte überprüft. Wurden dort gültige Vorhersagedaten
...
\item Mit dem Programm \textit{degrib} werden bestimmte Positionen auf
  ...
  \item Wurde auf dem Umkreis eine Position mit gültigen
    Vorhersagewerten gefunden, wird diese als alternative
    Vorhersageposition vorgemerkt, und die obere Schranke auf $u = u - (u
    - l)/2$ gesetzt.
...
\item Solange $u - l > 1m$ ist, wird mit Schritt 4
  ...
  Algorithmus keine alternative Vorhersageposition finden.
...
Richtungsschrittweite $\alpha$ von $10^{\circ}$ und einem Schwellwert
$d$ von einem Meter initialisiert. Da an der Ursprungsposition keine
gültigen Vorhersagewerte gefunden wurden, sucht der Algorithmus
zunächst nach gültigen Position auf dem Umkreis mit einem Radius von
50 Kilometern. In Zeile 2 kann man erkennen, dass auf diesem Umkreis
eine gültige Position gefunden wurde. Daher müssen Positionen, die
mehr als 50km von der Ursprungsposition entfernt sind, nicht weiter
untersucht werden. In den folgenden Iterationen werden weitere
Positionen untersucht und die untere oder obere Schranke entsprechend
angepasst. Nachdem der Schwellwert unterschritten wurde terminiert der
Algorithmus nach ca. 8 Sekunden mit einer alternativen
