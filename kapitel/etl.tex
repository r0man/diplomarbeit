...
beschrieben. Anschließend wird der Algorithmus durch Anwendung des
\textit{Divide and Conquer} Prinzips erweitert, und ein Verfahren
angewendet das gewisse Parallelen zu einer binären aufweist. Ziel des
Algorithmus ist es diejenige geographische Position zu finden, die dem
gegebenen Spot am nächsten liegt und für die Vorhersagedaten
existieren.
...
\item Bei der Initialisierung des Algorithmus wird die maximale
  ...
  $\Omega$ von der Ursprungsposition $\Theta$ des gegebenen Spots
  entfernt sein darf. Weiterhin eine Schrittweite $\delta < \Delta$,
  um die der Radius $\rho$ pro Iteration vom Ursprung aus erhöht wird,
  und der Abstand $\alpha < 360^{\circ}$ , welcher die Schrittweite
  der Richtung angibt.
...
\item Wurden an der/den bisher betrachteten Position(en) keine
  ...
  überschritten hat, ist der Algorithmus fertig und es wurde keine
  alternative Vorhersageposition gefunden.

\item Nach der Erhöhung des Radius werden bestimmte geographische
  Positionen auf dem Umkreis des Spots auf die Existenz gültiger
...
Ausgabe geparst und anschließend ausgewertet. Eine alternative
Vorhersageposition wurde genau dann gefunden, wenn alle Werte in der
Ausgabe von \textit{degrib} gültig sind. Die Anzahl der
Programmaufrufe ist mit den bei der Initialisierung festgelegten
Parametern durch die Obergrenze $1 + (360^{\circ} / \alpha) * (\Delta
/ \delta)$ beschränkt. 
...
3601$ mal ohne Erfolg aufgerufen. Bei allen anderen Spots war die
alternative Vorhersageposition zwischen 3 und 80 Kilometern von der
Ursprungsposition entfernt. 

\subsubsection{Verbesserung des Algorithmus}
Damit der Algorithmus zu einem Spot auch wirklich die nächste
...
werden. Ist diese Schrittweite zu hoch angesetzt, kann es vorkommen
dass eine in Wirklichkeit viel näher am Ursprung liegende alternative
Vorhersageposition nicht gefunden wird, da sie bei einer Iteration
übersprungen wurde. Wählt man die Schrittweite allerdings zu klein,
...
selbe gilt für die Wahl der Schrittweise $\alpha$, mit der die
verschiedenen Richtungen der zu suchenden Position berechnet wird.
